...
public class BNetPanel extends JPanel implements CytoPanelComponent {
    ...
    private void addInputListeners() {
        btnMotiffsConstructor.addActionListener(event -> {
        ...
        btnFixedPoints.addActionListener(arg0 -> {
            ...
            if (n >= 256)
                select = PopupMessage.ConfirmMessage("The number of all possible values is " + n + ".The execution time can be very long.");
            if (select != 0) return;
